...
#include "stb_image.h"
 ...
 std::vector<std::vector<std::string>>–есурсныйћенеджер::п_уровени;
 ...
 std::string –есурсныйћенеджер:: получить—троку‘айла(const std::string& относительный‘айлаѕути) 
{
	                                                                 ...
	                                                                 const std::string& расположение“екстур»м€ )
 {
	 ...
	 std::shared_ptr<ƒвижок–ендеринга::—прайт> нова€—прайт = п_—прайт.emplace(»м€—прайт, std::make_shared<ƒвижок–ендеринга::—прайт>(п“екстур, пЎейдор,
		                                                                                                         расположение“екстур»м€)).first->second;
	                                                                          ...
	                                                                          const unsigned int расположение“екстур¬ысота)
 {
	 ...
	 else if (п“екстур && ѕуть“екстуры == "ресерсы/текстуры/мир.png")
	 {
		 ...
		 for (auto& текущий–асположение“екстур»м€ : расположение“екстур)
		 {
			 ...
			 текущий–асположение“екстур¬ыкл—охранениеX += 49;
			 if (текущий–асположение“екстур¬ыкл—охранениеX == Ўиренатекстуры)
			 {
 ...
 bool –есурсныйћенеджер::загрускаJSON–есурсов(const std::string& JSONPѕуть)
 {
	 ...
	 if (текстурјтлос»т != документ.MemberEnd())
	 {
		 for (const auto& “екущий“екстурјтлос : текстурјтлос»т->value.GetArray())
		 {
			 ...
			 const auto  расположение“екстурћассив = “екущий“екстурјтлос["расположение“екстур»м€"].GetArray();
			 std::vector<std::string>расположение“екстур;
	 ...
	 auto спрайт = документ.FindMember("спрайт");
	 if (спрайт != документ.MemberEnd())
	 {
		 for (const auto& текущий—прайт : спрайт->value.GetArray())
		 {
			 const std::string им€ = текущий—прайт["им€"].GetString();
			 const std::string текстурјтлос = текущий—прайт["текстурјтлос"].GetString();
			 const std::string шейдор = текущий—прайт["шейдор"].GetString();
			 const std::string расположение—уб“екстур»м€ = текущий—прайт["расположение—уб“екстур»м€"].GetString();
			 auto п“анк—прайт = загруска—прайт(им€, текстурјтлос, шейдор, расположение—уб“екстур»м€);
			 if (!п“анк—прайт)
			 {
				 continue;
			 }
	 ...
	 if (анимаци€—прайт != документ.MemberEnd())
	 {
		 for (const auto& текущийјнимаци€—прайт : анимаци€—прайт->value.GetArray())
		 {
			 ...
			 auto п“анкјнимаци€—прайт = загрускајнимаци€—прайт(им€, им€“екстур, им€Ўедоры, расположение“екстур»м€);
	 ...
	 if (анимаци€—прайт1 != документ.MemberEnd())
	 {
		 for (const auto& текущийјнимаци€—прайт : анимаци€—прайт1->value.GetArray())
		 {
			 ...
			 const std::string им€Ўедоры = текущийјнимаци€—прайт["им€Ўедоры"].GetString();
			 const std::string расположение“екстур»м€ = текущийјнимаци€—прайт["расположение“екстур»м€"].GetString();
			 auto п“анкјнимаци€—прайт1 = загрускајнимаци€—прайт(им€, им€“екстур, им€Ўедоры, расположение“екстур»м€);
	 ...
	 if (уровень»т != документ.MemberEnd())
	 {
		 for (const auto& “екущий”ровеньрјтлос : уровень»т->value.GetArray())
		 {
			 ...
			 size_t максимальна€—трока = 0;
			 for (const auto& текущий–€д : описание)
			 {
				 уровень–€д.emplace_back(текущий–€д.GetString());
				 if (максимальна€—трока < уровень–€д.back().length())
				 {
					 максимальна€—трока = уровень–€д.back().length();
				 }
			 }			
			 for ( auto& текущий–€д : уровень–€д)
			 {
				 while (текущий–€д.length() < максимальна€—трока)
				 {
					 текущий–€д.append("ѕ");
				 }
			 }
			 п_уровени.emplace_back(std::move(уровень–€д));
		 }
	 }
