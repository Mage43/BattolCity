...
namespace Рендор {
	...
	void Текстуры2D::добавлятьРасположениеТекстур(std::string имя, const glm::vec2& СлеваВнизуUV, const glm::vec2& ПравоСверхуUV)
	{
		п_расположениеТекстур.emplace(std::move(имя), РасположениеТекстур2D(СлеваВнизуUV, ПравоСверхуUV));
	}

	const Текстуры2D::РасположениеТекстур2D& Текстуры2D::получитьРасположениеТекстур(const std::string& имя) const
	{
		auto это = п_расположениеТекстур.find(имя);
		if (это != п_расположениеТекстур.end())
		{
			return это->second;
		}
		const static РасположениеТекстур2D ПоумолчаниюРасположениеТекстур;
		return ПоумолчаниюРасположениеТекстур;
	}
}